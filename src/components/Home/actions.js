  ...
  return dispatch => {
    ...
    return axios
      ...
      .then(response => {
        ...
        dispatch({ type: actionTypes.HOME_MSG_LOAD_SUCCESS, data: response.data })
      })
      .catch(error => {
        dispatch({ type: LOADING_FINISH })
        if (error.response) {
          // status !== 2xx
          dispatch({ type: actionTypes.HOME_MSG_LOAD_FAIL, data: error.response.data })
        } else if (error.request) {
          // The request was made but no response was received
          // Handle this case base on your requirments
          dispatch({
            type: actionTypes.HOME_MSG_LOAD_FAIL,
